...
impl<Q: QuoteBook, L: Ledger + Clone + Sync + 'static> SynchronizedQuoteBookImpl<Q, L> {
    ...
    pub fn add_sci(
        ...
        .ledger
        .num_blocks()
        .map_err(|err| QuoteBookError::ImplementationSpecific(err.to_string()))? - 1;
...
impl<Q: QuoteBook, L: Ledger + Clone + Sync + 'static> SynchronizedQuoteBook<Q, L> {
    ...
    pub fn new(
        ...
        let synchronized_quote_book_impl = Arc::new(SynchronizedQuoteBookImpl::new(quote_book, ledger, remove_quote_callback, logger));
        Self {
...
where
    ...
    L: Ledger + Clone + Sync + 'static,
{
    fn add_sci(&self, sci: SignedContingentInput, timestamp: Option<u64>) -> Result<Quote, QuoteBookError> {
    ...
    fn remove_quotes_by_key_image(&self, key_image: &KeyImage) -> Result<Vec<Quote>, QuoteBookError> {
        self.synchronized_quote_book_impl.remove_quotes_by_key_image(key_image)
    }
    ...
    fn remove_quotes_by_tombstone_block(
        ...
        self.synchronized_quote_book_impl.remove_quotes_by_tombstone_block(current_block_index)
    }
    ...
    fn get_quotes(
        ...
        self.synchronized_quote_book_impl.get_quotes(pair, base_token_quantity, limit)
    }
