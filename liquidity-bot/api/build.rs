...
fn main() {
    ...
    let deqs_api_proto_path = env
        .depvar("DEQS_API_PROTOS_PATH")
        .expect("Could not read api's protos path")
        .to_owned();
    let deqs_api_proto_dirs = deqs_api_proto_path.split(':').collect::<Vec<&str>>();
    all_proto_dirs.extend(deqs_api_proto_dirs);
