...
type ListenerRequestsMap = HashMap<ListenerId, oneshot::Sender<Result<Multiaddr, ClientError>>>;
...
pub struct NetworkEventLoop<REQ: RpcRequest, RESP: RpcResponse> {
    ...
    /// A map of pending listener requests for which we are still waiting on.
    listener_requests: ListenerRequestsMap,
}
    ...
    async fn handle_swarm_event<TErr: Debug>(
        ...
        match event {
            ...
            SwarmEvent::NewListenAddr {
                ...
                match self.listener_requests.remove(&listener_id) {
                    Some(sender) => {
                        sender
                            .send(Ok(address.clone()))
                            .expect("receiver should not be dropped");
                    }
                    None => {
                        log::warn!(
                            &self.logger,
                            "no listener request for {:?}/{:?}",
                            address,
                            listener_id
                        );
                    }
                ...
                QueryResult::GetProviders(Ok(ok)) if ok.key == self.peer_discovery_key => {
                    for peer in ok.providers {
                        ...
                        log::info!(
                            ...
                            peer,
                            std::str::from_utf8(ok.key.as_ref()).unwrap_or("<invalid>"),
                            addrs,
                        );
                    }
                }
    ...
    async fn handle_command(&mut self, command: Command<REQ, RESP>) {
        match command {
            ...
            Command::Listen {
                ...
                Ok(listener_id) => {
                    self.listener_requests.insert(listener_id, response_sender);
                }
